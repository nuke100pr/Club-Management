...
const EventsPage = () => {
      ...
      try {
        ...
        if (userData) {
          ...
          if (userData.userData?.clubs) {
            ...
            ).filter((clubId) => userData.userData.clubs[clubId].events === true);
        ...
        if (eventId) {
          ...
          if (userData && Array.isArray(rsvpResult.data)) {
            setIsRegistered(rsvpResult.data.some(r =>
              r.user_id === userData.userId ||
              (typeof r.user_id === 'object' && r.user_id?._id === userData.userId)
            ));
          }
        }
  ...
  const handleDeleteRsvp = async (rsvpId) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/events/${eventId}/rsvp/${rsvpId}`, {
            method: 'DELETE'
          });
          ...
          if (!result.success) throw new Error(result.message || 'Failed to delete RSVP');
  ...
  const handleEdit = async () => {
    try {
      ...
      if (!result.success) throw new Error(result.message || 'Failed to fetch event');
