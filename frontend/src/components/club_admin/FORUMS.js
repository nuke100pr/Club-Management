...
const ForumList = ({
  ...
  const [selectedBoard, setSelectedBoard] = useState(propclubId);
      ...
      if (result) {
        ...
        if (result.userData?.data?.boards) {
          const boardsWithPermission = Object.keys(
            ...
            (clubId) => result.userData.data.boards[clubId].forums === true
          );
  ...
  const filteredForums = forums.filter(
    (forum) =>
      ...
      (propclubId ? forum.club_id === propclubId : true) &&
      (!selectedBoard || forum.club_id === selectedBoard) &&
    ...
    <div>
      <Box sx={{ position: "relative", minHeight: "100vh", pb: 8 }}>
        <SearchAndFilter
          ...
          disableBoardFilter={!!propclubId}
          initialBoard={propclubId}
        />
          ...
          {filteredForums.length > 0 ? (
            filteredForums.map((forum) => (
              <Grid item key={forum._id}>
                <ForumCard
                  forum={forum}
                  boardName={availableBoards[forum.club_id]}
        ...
        {createForumOpen && (
          <ForumCreateDialog
            ...
            userId={userId}
            club_id={propclubId || selectedBoard}
            club_id={selectedClub}
            disableBoardSelection={!!propclubId}
          />
        )}
