...
async function filterEventsByclubId(eventsData, targetclubId) {
  try {
    ...
    // Filter events where:
    // 1. club_id exists and matches the target ID (direct board assignment)
    // OR
    // 2. club_id exists and its club_id matches the target ID (club's board assignment)
    const filtered = eventsData.data.filter((event) => {
      const hasDirectBoardMatch =
        event.club_id && event.club_id._id === targetclubId;
...
const CalendarView = ({ clubId }) => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        console.log(result);
        const filteredEvents = await filterEventsByclubId(
          result,
          "67eee9b433cf4e1c7f289345"
        );
  ...
  const filteredEvents = transformedEvents.filter(
    (event) =>
      ...
      (!selectedBoard || event.club_id === selectedBoard) &&
  ...
  const boards = [...new Set(events.map((event) => event.club_id))];
        ...
        <Container maxWidth="xl" sx={{ py: 2, height: "100%" }}>
          ...
          >
            <TextField
              ...
              InputProps={{
                endAdornment: (
                  <IconButton onClick={handleFilterToggle} edge="end">
                    <FilterList />
                  </IconButton>
                ),
              }}
