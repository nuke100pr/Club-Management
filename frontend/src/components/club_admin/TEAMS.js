...
export default function BoardPrivileges({ clubId }) {
    ...
    const fetchPrivileges = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/misc/misc/club/${clubId}`
        );
  ...
  if (
    !privileges ||
    !privileges.data.users ||
    privileges.data.users.length === 0
  ) {
    return (
    ...
    <Paper elevation={3} sx={{ overflow: "hidden" }}>
      <Box p={3} borderBottom="1px solid" borderColor="divider">
        ...
        <Typography variant="body2" color="text.secondary" mt={1}>
          Board ID: {privileges.data.clubId}
        </Typography>
      </Box>
      ...
      >
        <Typography variant="caption" color="text.secondary">
          Showing {privileges.data.users.length} user
          {privileges.data.users.length !== 1 ? "s" : ""}
        </Typography>
      </Box>
    </Paper>
  );
