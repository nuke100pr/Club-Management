...
const UniversalShareMenu = ({ 
  ...
  const getPathSegment = () => {
    switch(contentType) {
      case 'event': return 'current_event';
      case 'post': return 'current_post';
      case 'project': return 'current_project';
      case 'blog' : return 'current_blog';
      default: return contentType + 's'; // fallback to pluralizing
    }
  };
  ...
  const shareViaX = () => {
    ...
    window.open(`https://x.com/intent/tweet?text=${text}&url=${encodeURIComponent(shareUrl)}`);
    onClose();
  };
  ...
  const useNativeShare = () => {
    if (navigator.share) {
      ...
      showNotification('Native sharing not supported on this device', 'warning');
    }
    onClose();
  };
    ...
    <>
      ...
      >
        ...
        {navigator.share && (
          <MenuItem onClick={useNativeShare}>
            ...
            <ListItemText primary="Share via device" secondary="Use your device's sharing" />
          </MenuItem>
        )}
      ...
      >
        <Alert 
          ...
          action={
            <Close fontSize="small" onClick={closeSnackbar} sx={{ cursor: 'pointer' }} />
          }
        >
          {snackbar.message}
        </Alert>
