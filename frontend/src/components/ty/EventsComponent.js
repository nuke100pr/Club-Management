...
export default function EventsPage({ boardId = null, searchQuery = "" }) {
          ...
          (result.data || []).map(async (event) => {
            try {
              ...
              return {
                ...
                registered:
                  rsvpResult.data?.some((reg) => reg.user_id === user_id) ||
                  false,
              };
            } catch (error) {
              console.error(
  ...
  return (
    <Box sx={{ minHeight: "100vh", p: 4 }}>
            ...
            <List>
              {registrationsDialog.registrations.map((registration) => (
                <ListItem key={registration._id}>
                  <ListItemAvatar>
                    <Avatar src={registration.user_id?.profile_picture} />
                  </ListItemAvatar>
                  <ListItemText
                    primary={registration.user_id?.name}
                    secondary={registration.user_id?.email_id}
                  />
                </ListItem>
              ))}
            </List>
      ...
      {/* Event Form Dialog - You'll need to create this component */}
      <EventForm
        open={openDialog}
        ...
        userClubs={userClubsWithEventPermission}
        userBoards={userBoardsWithEventPermission}
